...
- **`@scope`**: is just concept for **local** `singleton`, scope works as **code validation** in **compile time**
  + Code validation: if your subcomponent declares scope of parent components, the error is occurred.
  + If we add `@scope` to our component, then we inject an `unscope` object, it will be re-created every injection times. If we inject `@scope` marked with object, it will retain same object like singleton behavior. And everything will be re-created if the component re-created.
